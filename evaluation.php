        ...
        <div id="customizeSection" class="mt-4" style="display: none;">
            ...
            <div id="resultSection" style="display: none;">
                ...
                <form id="budgetForm" action="report-custom" method="POST" style="display: none;">
                    ...
                    $adjusted_quantities = []; // Array to store adjusted quantities

                    while ($row = $ingredients_result->fetch_assoc()) {
                        $ingredient_id = $row['id'];
                        $ingredient_quantity = $row['quantity'];
                        $ingredient_name = htmlspecialchars($row['ingredient_name']);
                        echo "<input type='hidden' name='ingredient_ids[]' value='{$ingredient_id}'>";
                        echo "<input type='hidden' name='ingredient_quantities[]' value='{$ingredient_quantity}'>";
                        echo "<input type='hidden' name='ingredient_names[]' value='{$ingredient_name}'>";

                        // Include hidden inputs for adjusted quantities
                        $adjusted_quantities[] = $ingredient_quantity; // Store initial quantity (will be adjusted in JS)
                    }
                    ?>
        ...
        $('#quantityInput').on('input', function() {
            ...
            if (!isNaN(inputQuantity) && selectedId) {
                ...
                ingredients.forEach(ingredient => {
                    ...
                    // Include hidden inputs to pass adjusted quantities to the next form
                    $('#budgetInputs').append(`
                        <input type="hidden" name="adjusted_quantities[]" value="${adjustedQuantity}">
                    `);
                });
