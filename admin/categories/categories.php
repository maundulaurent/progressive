...
// Fetch categories and industries from the database
$query = "SELECT category.id, category.name AS category_name, industry.id AS industry_id, industry.name AS industry_name 
          FROM category 
          JOIN industry ON category.industry_id = industry.id 
          ORDER BY category.id ASC";

$result = $conn->query($query);

$industryQuery = "SELECT * FROM industry ORDER BY name ASC";
											...
											<?php if ($result->num_rows > 0): ?>
												<?php $s_no = 1; while ($row = $result->fetch_assoc()): ?>
													<tr>
														...
														$industryId = htmlspecialchars($row['industry_id']);  // This should now exist
														?>
														
															<a href="javascript:void(0);" class="btn btn-sm btn-secondary me-2"
																data-bs-toggle="modal" data-bs-target="#edit-category"
																data-id="<?php echo $categoryId; ?>"
																data-name="<?php echo $categoryName; ?>"
																data-industry-id="<?php echo $industryId; ?>">
                    ...
                    <form action="add.php" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="industry_id" class="form-control form-select" required>
                                ...
                                <?php while ($industry = $industryResult->fetch_assoc()): ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="edit.php" method="POST">
                        ...
                        <div class="form-group">
                            <label>Select Industry</label>
                            <select name="industry_id" id="edit-industry-id" class="form-control form-select" required>
                                <option value="">Select Industry</option>
                                <?php
                                $industryResult->data_seek(0); // Reset pointer to beginning
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            $('#edit-category').on('show.bs.modal', function (event) {
                ...
                var industryId = button.data('industry-id');
